...
// --- INITIAL DATA WITH "25% DROP & RECOVER" MOCKUP ---
// Household Income labels updated to short format (5k-10k)
...
AP,household_income,"< 5k",14978,15047,15922,16100,16416,16912,12684,12854,13024,13194,13606
AP,household_income,"5k-10k",10410,10870,11305,11489,11822,12466,9349,9519,9689,9859,11432
AP,household_income,"10k-20k",18742,17457,18506,18856,19631,20853,15640,15810,15980,16150,13708
AP,household_income,"20k-30k",10579,10117,10357,10433,10881,11586,8689,8859,9029,9199,6414
AP,household_income,"30k-50k",11440,10749,11024,11106,11414,12057,9043,9213,9383,9553,5015
AP,household_income,"50k-75k",5373,5279,5379,5427,5608,5871,4403,4573,4743,4913,2278
AP,household_income,"75k-100k",1693,1729,1752,1777,1775,1909,1432,1602,1772,1942,773
AP,household_income,"100k-150k",1518,1452,1459,1457,1488,1561,1171,1341,1511,1681,620
AP,household_income,"> 150k",1357,1263,1310,1325,1321,1392,1044,1214,1384,1554,654
`;
...
const Dashboard = () => {
  ...
  const processData = (csvText: string) => {
    try {
      ...
      const uniqueTopics = Array.from(new Set(rawData.map((row: any) => row.Topic)));
      setTopics(['Overview', ...uniqueTopics]); 
  ...
  const aggregatedData = useMemo(() => {
    ...
    // Custom Sorting & Labeling for Income
  ...
  const topicTrendData = useMemo(() => {
    if (months.length === 0 || aggregatedData.length === 0) return [];
    return months.map(m => {
  ...
  const overviewSESData = useMemo(() => {
    ...
    const sesRules = {
      'SES A (>75k)': ['75k-100k', '100k-150k', '> 150k'], 
      'SES B (30k-75k)': ['30k-50k', '50k-75k'], 
      'SES C (10k-30k)': ['10k-20k', '20k-30k'], 
      'SES D (5k-10k)': ['5k-10k'], 
      'SES E (<5k)': ['< 5k'] 
    };
    ...
    filtered.forEach((row: any) => {
      // Use Segment directly since we updated Initial Data to short labels
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex bg-slate-100 p-1 rounded-lg inline-flex">
                ...
                {panels.map(p => (
                  <button key={p} onClick={() => setSelectedPanel(p)} className={`px-4 py-1.5 text-sm rounded-md font-medium transition-all ${selectedPanel === p ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{p}</button>
                ))}
              </div>
            </div>
      ...
      {selectedTopic === 'Overview' ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
            <h3 className="text-lg font-bold text-slate-700 mb-4 flex items-center gap-2">
              <MapPin className="w-5 h-5 text-red-500" /> Regional Distribution ({monthA})
            </h3>
          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
            <h3 className="text-lg font-bold text-slate-700 mb-4 flex items-center gap-2">
              <Users className="w-5 h-5 text-green-500" /> Age Group Distribution ({monthA})
            </h3>
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={overviewAgeData}
                    cx="50%"
                    cy="50%"
                    innerRadius={60}
                    outerRadius={80}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:col-span-2">
            <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
              <h3 className="text-lg font-bold text-slate-700 mb-4 flex items-center gap-2">
                <Wallet className="w-5 h-5 text-yellow-500" /> Household Income by SES ({monthA})
              </h3>
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
              <h3 className="text-lg font-bold text-slate-700 mb-4 flex items-center gap-2">
                <Briefcase className="w-5 h-5 text-indigo-500" /> Top Employment Status ({monthA})
              </h3>
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Left: Stats Table */}
          <div className="lg:col-span-1 bg-white p-5 rounded-xl shadow-sm border border-slate-100 overflow-hidden flex flex-col">
            <div className="mb-4">
